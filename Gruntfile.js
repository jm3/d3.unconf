  ...
  grunt.initConfig({
    ...
    connect: {
      ...
      development: {
        options: {
          middleware: function(connect) {
            return [
              require('connect-livereload')(), connect.static(require('path').resolve(config.paths.srv)), connect.static(require('path').resolve(config.paths.src))
            ];
          }
        }
      },
      ...
      build: { // serve the built bundle, mimicks live loading behavior with a real webserver
        options: {
          middleware: function(connect) {
            return [connect.static(require('path').resolve(config.paths.dist))];
          },
          keepalive: true
        }
      }
    ...
    // , compress : { options : { mode : 'gzip' } ,
    // production : { files : [{ expand : true , cwd  : config.paths.dist ,
    // src  : ['**/*', '!**/*.{gif,jpg,jpeg,png}'] , dest  : config.paths.dist }] } }
    ...
    clean: {
      build: {
        files: [{
          dot: true,
          src: [
            config.paths.tmp, config.paths.dist
          ]
        }]
      },
    ...
    copy: {
      build: {
        files: [{
          ...
          cwd: config.paths.src,
          src: [
            '**/*.{gif,webp,ico,json,woff,ttf,eot,svg,swf}', '!stylesheets/**/*', '!vendor/bootstrap/fonts/**/*'
          ],
